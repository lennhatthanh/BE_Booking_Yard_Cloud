  ...
  async thanhToan(req, res) {
    try {
      ...
      const order = {
        ...
        orderCode: orderCode,
        expiredAt,
        returnUrl: `http://localhost:3000/dat-san/check-thanh-toan/${id_san}`,
        cancelUrl: `http://localhost:3000/dat-san/check-thanh-toan/${id_san}`,
      };
  ...
  async checkThanhToan(req, res) {
    try {
      ...
      if (cancel === "true" || status === "CANCELLED") {
        ...
        return res.redirect(`http://localhost:5173/dat-san/${id_san}`);
      }
