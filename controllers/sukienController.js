...
class sukienController {
  ...
  async themMoiSuKien(req, res) {
    try {
      ...
      const gio_bat_dau = parseInt(
        thoi_gian_bat_dau.split("T")[1].split(":")[0]
      );
      const gio_ket_thuc = parseInt(
        thoi_gian_ket_thuc.split("T")[1].split(":")[0]
      );
      let gio = [];
      let ngay = [];
  ...
  async thamGiaSuKien(req, res) {
    try {
      const { id_su_kien, can_cuoc_cong_dan, phi_tham_gia, so_luong } =
        req.body;
      ...
      const data = await nguoidungdungsukiens.add(
        req.user.id,
        id_su_kien,
        can_cuoc_cong_dan,
        orderCode
      ...
      const order = {
        ...
        orderCode: orderCode,
        expiredAt,
        returnUrl: `http://localhost:3000/su-kien/check-thanh-toan`,
        cancelUrl: `http://localhost:3000/su-kien/check-thanh-toan`,
      };
  ...
  async checkThanhToan(req, res) {
    try {
      ...
      if (cancel === "true" || status === "CANCELLED") {
        ...
        return res.redirect(`http://localhost:5173/su-kien`);
      }
  ...
  async getAllOpen(req, res) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Lấy dữ liệu thành công", data, sukien });
    } catch (error) {
      console.log(error);
      return res.status(500).json({ message: "Lỗi: " + error.message });
    }
